...
-- ═══════════════════════════════════════════════════════════════════════════════
-- FOLDED-IN CORRECTIONS
-- ═══════════════════════════════════════════════════════════════════════════════
-- Everything below re-creates functions defined earlier in this same file, with
-- three faults fixed that only showed up once real orders ran through them:
--
--   · an anonymous caller could reach assign_delivery_token, because an
--     ownership test against a NULL auth.uid() yields NULL, which Postgres
--     treats as false rather than as a failure
--   · orders.business_id is text while every table here uses uuid, so any
--     comparison between them had no operator
--   · driver_lookup returns a column named `token` and also filtered on
--     orders.token, which is ambiguous inside plpgsql
--
-- They live at the end rather than replacing the originals in place: CREATE OR
-- REPLACE means the last definition wins, and keeping the first version visible
-- above shows what was wrong with it.
--
-- An existing database can run FIX_003 and FIX_004 instead of this whole file.
-- ═══════════════════════════════════════════════════════════════════════════════
